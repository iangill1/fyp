...
def xgboost_sentiment_forecast(price_data, news_sentiment):
    data = price_data.copy()
...
def xgboost_step_forecast(price_data):
    data = price_data.copy()
    ...
    #predictions = []
    model = XGBRegressor(random_state=42).fit(train_exog, train_close)
...
def xgboost_sentiment_step_forecast(price_data, news_sentiment, *, roll_window_days=3, lag_days=1, multiplier=0.0199918789,):
    data = price_data.copy()
    ...
    model = XGBRegressor(**best_params, random_state=42).fit(train_exog, train_close)
    base_pred = pd.Series(
